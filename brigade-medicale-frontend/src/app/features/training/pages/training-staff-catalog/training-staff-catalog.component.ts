...
import { TrainingModule, ProgressStatus, TrainingAudience, TrainingProgress } from '../../core/models/training.models';
import { TrainingService } from '../../core/services/training.service';
import { TrainingStorageService } from '../../core/services/training-storage.service';
import { AuthService } from '../../../../core/auth/services/auth.service';

@Component({
  selector: 'app-training-staff-catalog',
  standalone: true,
      ...
      <div class="bg-white border-b border-gray-200 p-4 sm:p-6">
        <div class="max-w-6xl mx-auto">
          ...
          <p class="text-gray-600 text-sm sm:text-base">
            Améliorez vos connaissances et compétences
          </p>
          ...
          @if (filteredModules.length > 0 && !isLoading && !error) {
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              @for (module of filteredModules; track module.id) {
                ...
                >
                  <!-- Module Image -->
                  <div class="h-40 sm:h-48 bg-gradient-to-br from-blue-100 to-indigo-100 overflow-hidden">
                    @if (module.imageUrl) {
                      <img
                        [src]="module.imageUrl"
                        [alt]="module.title"
                  ...
                  <div class="p-4 sm:p-5">
                    ...
                    <!-- Level & Tags -->
                    <div class="flex items-center justify-between mb-3">
                      <span class="text-xs font-medium text-blue-600 bg-blue-50 px-2.5 py-1 rounded">
                        {{ module.level }}
                      </span>
                    ...
                    @if (getModuleStatus(module.id) !== 'NOT_STARTED') {
                      <div class="mb-3">
                        <div class="flex items-center justify-between mb-1">
                          ...
                          <span class="text-xs font-medium text-blue-600">{{ getModuleScore(module.id) }}%</span>
                        </div>
...
export class TrainingStaffCatalogComponent implements OnInit {
  searchTerm = '';
  ...
  ngOnInit(): void {
    this.authService.getCurrentUser().pipe(
      ...
      if (user && user.roles && user.roles.length > 0) {
        // Use the first role to determine audience
  ...
  private loadModules(): void {
    this.isLoading = true;
    this.error = null;
    this.trainingService.getModulesByAudience(this.audience).pipe(
      takeUntilDestroyed(this.destroyRef)
    ).subscribe({
      next: (modules) => {
        ...
        this.trainingStorage.setAudience(this.audience);
        this.updateStats();
  ...
  getModuleScore(moduleId: string): number {
    ...
    if (progress.completedSteps?.length) {
      return Math.min(100, Math.round((progress.completedSteps.length / 10) * 100));
    }
    return progress.status === 'IN_PROGRESS' ? 25 : 0;
  }
  ...
  getCTAButtonClass(status: ProgressStatus): string {
    const classes: Record<ProgressStatus, string> = {
      'NOT_STARTED': 'bg-blue-600 text-white hover:bg-blue-700',
      'IN_PROGRESS': 'bg-indigo-600 text-white hover:bg-indigo-700',
  ...
  navigateToModule(moduleId: string): void {
    ...
    this.router.navigate(['/training', moduleId]);
  }
}
